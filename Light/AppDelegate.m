...
//            [application cancelLocalNotification:alarmNotification];
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(checkBatteryState) name:UIDeviceBatteryLevelDidChangeNotification object:nil];
    ...
    //////////////--------background mode refresh status check
    UIAlertView * alert;
...
- (BOOL)application:(UIApplication *)application handleOpenURL:(NSURL *)url {
    ...
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Test" message:@"OK" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alertView show];
...
//    [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"uplamp-new://"]];
    ...
    if ([[UIDevice currentDevice] batteryState] == UIDeviceBatteryStateUnplugged) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithBool:NO] forKey:CONNECTED_CHARGER];
//        [self pushLocalNotificationForLogWithText:@"Uplamp is ready!"];
        
    }else if ([[NSUserDefaults standardUserDefaults] objectForKey:CONNECTED_CHARGER]) {
        
        self.isConnectedCharger = YES;
        
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:CONNECTED_CHARGER] boolValue] == NO) {
            [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithBool:YES] forKey:CONNECTED_CHARGER];
            [self pushLocalNotificationForLogWithText:@"Uplamp is ready!"];
        }
...
- (void)pushLocalNotificationForLogWithText: (NSString *)body {
    ...
    if (notification) {
        [notification setRepeatInterval:0.0f];
        [notification setAlertBody:body];//[NSString stringWithFormat:@"%@: %@", body, [NSDate date]]
